...
#7. remover as 20% piores features com base no teste de kruskal wallis e na roc auc
...
for feature_selection_type, fs_X in feature_selection_Xs.items():
    ...
    #10. Loop de experiências com feature selection + data processing + classifier
    for data_processing_type, processed_X in data_processing_Xs.items():
        ...
        #11. verificar o melhor K para o classifier KNN
      
        print_log("TESTING KNN...")
        knn_classifier = KnnClassifier(data_info = data_info_string)
        knn_training_results = knn_classifier.knn_analysis(processed_X, Y, n_runs=2)

        # Guardar resultados
        df = pd.DataFrame(zip(*knn_training_results))
        df.columns = ['K', 'Average Error', 'STD Error']  # adjust as needed
        ...
        svm_training_results = svm_classifier.svm_analysis(processed_X, Y, n_runs=2)

        # Guardar resultados
        df = pd.DataFrame(zip(*svm_training_results))
        df.columns = ['C', 'Average Error', 'STD Error']  # adjust as needed
        df.to_csv(f"svm_training/err_{data_info_string}.csv", index=False)

        #13. Selecionar quais classificadores usar com base no dataset
